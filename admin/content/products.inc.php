...
$pageBase = '/?cmd=products';

$query = "SELECT id FROM productCategories";
$result = mysql_query($query);
if($result){
	$hasCategories = TRUE;
}

$query = "SELECT projectTitle FROM products";
$result = mysql_query($query);
if($result){
	$hasTitle = TRUE;
}

$query = "SELECT projectCost FROM products";
$result = mysql_query($query);
if($result){
	$hasCost = TRUE;
}

$query = "SELECT pdf FROM products";
$result = mysql_query($query);
if($result){
	$hasPDF = TRUE;
}
...
if($hasCategories){
	function get_categories(){
		$query = "SELECT name, id FROM productCategories ORDER BY name";
		$result = mysql_query($query);
		$array = array();
...
$dir = '../images/products/';   // Path To Images Directory
$dir2 = '../images/products/thumbs/';   // Path To Thumbs Directory
$dir3 = '../images/products/pdf/';   // Path To pdf Directory
...
function delete_pdf($id,$dir3){
	//check to see if current image exists
	$q = "SELECT pdf FROM products WHERE id='$id'";
	$res = mysql_query($q);
	$rec = mysql_fetch_assoc($res);
	if($rec['pdf'] != ''){
...
if($_POST['action'] == 'process'){
	...
	if($passCheck){
		...
		if($id == ''){
			$query = "INSERT INTO products SET $vals";
		}else{
			$query = "UPDATE products SET $vals WHERE id='$id'";
		}
	...
	if($_POST['catsort']){
		...
		for ($i = 0, $l = count($_POST['sortorder']); $i < $l; $i++) {
			...
			if($updatedClientID!= ''){
				$newOrderNum = $i*10;
				$query = "UPDATE products SET sortOrder='$newOrderNum' WHERE id=".$_POST['sortorder'][$i]." AND category = '".$_POST['catsort']."'";
		...
		for ($i = 0, $l = count($_POST['sortorder']); $i < $l; $i++) {
			...
			if($updatedClientID!= ''){
				$newOrderNum = $i*10;
				$query = "UPDATE products SET sortOrder='$newOrderNum' WHERE id=".$_POST['sortorder'][$i]." ";
	...
	if($passCheck){
		...
		if($id == ''){
			$query = "INSERT INTO productCategories SET $vals";
		}else{
			$query = "UPDATE productCategories SET $vals WHERE id='$id'";
		}
		if(mysql_query($query)){
			$msg .= 'Information updated successfully.<br />';
			//reget campains
	...
	if($item == 'category'){
		$query = "UPDATE productCategories SET active='0' WHERE id='$id'";
	...
	if($item == 'category'){
		$query = "UPDATE productCategories SET active='1' WHERE id='$id'";
	...
	if($item == 'category'){
		$query = "DELETE FROM productCategories WHERE id='$id'";
		$query2 = "DELETE FROM products WHERE category='$id'";
		
		$q = "SELECT id FROM products WHERE category = '$id'";
		$res = mysql_query($q);
