...
if($allowed){
	if($_GET['action'] == 'edit' && is_numeric($_GET['id'])){
		...
		$body .= '<h3>Edit a '.$functionName.'</h3>';
		...
		$body .= '
			<form action="'.$pageBase.'" method="post" enctype="multipart/form-data">';
				...
				if($hasTitle){
					$body .= '<label>Project Title: <br />
					<input type="text" class="text" name="projectTitle" value="'.$record['projectTitle'].'"/></label>';
				}
				
				if($hasName){
					$body .= '<label>Client Name: <br />
					<input type="text" class="text" name="client" value="'.$record['client'].'"/></label>';
				}
				
				if($hasLocation){
					$body .= '<label>Location: <br />
					<input type="text" class="text" name="location" value="'.$record['location'].'"/></label>';
				}
				
				if($hasTime){
					$body .= '<label>Time Frame: <br />
					<input type="text" class="text" name="timeframe" value="'.$record['timeframe'].'"/></label>';
				}
			
				$body .= '<label>'.$functionName.' Name:<br />
		...
		if($hasPDF){
			...
			<a href="'.$site.'/images/clients/pdf/'.$record['pdf'].'" target="_blank" >Click to open current specsheet</a>
			';
			} else{
				$body .= '<br>this project does not have a specsheet.</p>';
			}
		}
		if($hasImage){
			$body .= '
			<h2>Current Photo:</h2>
			<img src="'.$site.'/images/clients/'.$record['image'].'" style="max-width:740px;" />
			';
		}
		if($hasThumbnail){
			$body .= '
			<h2>Current Thumbnail:</h2>
			<img src="'.$site.'/images/clients/thumbs/'.$record['thumbnail'].'" />
			';
		}
		...
		$id = clean($_GET['id']);
		$query = "SELECT * FROM clientCategories WHERE id='$id' LIMIT 1";
		$result = mysql_query($query);
		$record = mysql_fetch_assoc($result);
		...
		$body .= '
			...
			<a onclick="return confirm(\'Are you sure you want to delete ' . $record['name'] . '? This will also delete any clients in this category.\')" href="'.$pageBase.'&action=delete&item=category&id='.$record['id'].'" title="Delete This Category" class="delete button-fix">Delete</a>
		';
	
	}else{
		//show clients
		$query = "SELECT * FROM clients ORDER BY category, name";
		$result = mysql_query($query);
		
		$body .= '
		<div id="tabs">
            <ul>
                <li><a href="#manage">Manage Existing '.$functionName.'</a></li>
                <li><a href="#add">Add a '.$functionName.'</a></li>';
            $body .= '</ul>';
            $body .= '<div id="manage">
		<p class="directions">Below your current clients are listed by category. The disabled clients / categories will have strikethroughs. Use the icons to the right to edit, disable or delete any item.</p>
		<ul class="page-list">
		';
		...
		while($record = mysql_fetch_assoc($result)){
			if($hasCategories && $currentCat != $record['category']){
				$q = "SELECT active FROM clientCategories WHERE id='".$record['category']."'";
				$res = mysql_query($q);
				$rec = mysql_fetch_assoc($res);
				
				$active = $rec['active'];
				$body .= '<li style="background: none; border:none; padding-top:10px;"><strong'; if($active == '0'){ $body .= ' class="inactive"'; } $body .= '>'.$categories[$record['category']].'</strong>';
			...
			if($hasTitle){
				...
				$body .= '<li><a class="mid'; if($active == '0' || $item_active == '0'){ $body .= ' inactive'; } $body .= '" title="Edit this client" href="'.$pageBase.'&action=edit&id='.$record['id'].'">'.$record['projectTitle'].'</a>';
			
				$body .=' <a onclick="return confirm(\'Are you sure you want to delete ' . $record['projectTitle'] . '?\')" class="icon" href="'.$pageBase.'&action=delete&item=client&id='.$record['id'].'" title="Delete This '.$functionName.'"><img width="15" class="icon" src="/images/icon_trash.png" alt="Delete" /></a>';
					
				if($item_active == '1'){
					$body .= ' <a onclick="return confirm(\'Are you sure you want to disable ' . $record['projectTitle'] . '?\')" class="icon" href="'.$pageBase.'&action=disable&item=client&id='.$record['id'].'" title="Disable This '.$functionName.'"><img width="15" class="icon" src="/images/icon_disable.png" alt="Disable" /></a>';
				}else{
					$body .= ' <a onclick="return confirm(\'Are you sure you want to enable ' . $record['projectTitle'] . '?\')" class="icon" href="'.$pageBase.'&action=enable&item=client&id='.$record['id'].'" title="Enable This '.$functionName.'"><img width="15" class="icon" src="/images/icon_enable.png" alt="Enable" /></a>';
				}
				
				$body .=' <a class="icon" href="'.$pageBase.'&action=clietn&id='.$record['id'].'" title="Edit This '.$functionName.'"><img width="15" class="icon" src="/images/icon_edit.png" alt="Edit" /></a>';
		...
		$q = "SELECT id, name, active FROM clientCategories ORDER BY name";
		$res = mysql_query($q);
		...
		$body .= '
			<form action="'.$pageBase.'" method="post" enctype="multipart/form-data">';
				...
				if($hasTitle){
					$body .= '<label>Project Title: <br />
					<input type="text" class="text" name="projectTitle" value="'.$_POST['projectTitle'].'"/></label>';
				}
				
				if($hasName){
					$body .= '<label>Client Name: <br />
					<input type="text" class="text" name="client" value="'.$_POST['client'].'"/></label>';
				}
				
				if($hasLocation){
					$body .= '<label>Location: <br />
					<input type="text" class="text" name="location" value="'.$_POST['location'].'"/></label>';
				}
				
				if($hasTime){
					$body .= '<label>Time Frame: <br />
					<input type="text" class="text" name="timeframe" value="'.$_POST['timeframe'].'"/></label>';
				}
				...
				if($hasCost){
					$body .= '<label>Project Cost: <br />
					<input type="text" class="text" name="projectCost" value="'.$_POST['projectCost'].'"/></label>';
				}
				
                $body .= '<label>Info:</label>
                <textarea name="info">'.stripslashes($_POST['info']).'</textarea>
				<label style="display:inline-block; width: 250px;">Is this '.$functionName.' active?</label> <label class="radio">yes <input title="Is this client active?" type="radio" name="active"'; if($_POST['active'] != '0'){ $body .= 'checked '; } $body .= 'value="1" /></label> <label class="radio">no <input title="Is this '.$functionName.' active?" type="radio" name="active"'; if($_POST['active'] == '0'){ $body .= 'checked '; } $body .= 'value="0" /></label>';
	...
	echo '<p>You have no clients for your site, please <a href="/">click here</a> to continue.</p>';
}
